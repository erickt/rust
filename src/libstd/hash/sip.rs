...
use super::{Hasher, StreamState, StreamHash};
...
impl Hasher<SipState> for SipHasher {
    /// Create a `SipState` that can be used for hashing a value.
    #[inline]
    fn state(&self) -> SipState {
        self.state
    }
}
...
impl StreamState for SipState {
    #[inline]
    fn input_bytes(&mut self, msg: &[u8]) {
        let length = msg.len();
        self.length += length;
...
#[inline]
pub fn hash<T: StreamHash<SipState>>(value: &T) -> u64 {
    hash_with_keys(0, 0, value)
}

/// Hash a value with the SipHash algorithm with the provided keys.
#[inline]
pub fn hash_with_keys<T: StreamHash<SipState>>(k0: u64, k1: u64, value: &T) -> u64 {
    let mut state = SipState::new(k0, k1);
    value.input(&mut state);
    state.result()
}
...
mod tests {
    ...
    use super::super::{Hash, StreamState, StreamHash};
    use super::{SipState, hash, hash_with_keys};

    // Hash just the bytes of the slice, without length prefix
    struct Bytes<'a>(&'a [u8]);

    impl<'a, S: StreamState> Hash<S> for Bytes<'a> {
    ...
    impl<'a, S: StreamState> StreamHash<S> for Bytes<'a> {
        #[inline]
    ...
    fn test_float_hashes_differ() {
        assert!(hash(&0.0f32) != hash(&1.0f32));
        assert!(hash(&1.0f32) != hash(&-1.0f32));
    }

    #[test]
    fn test_float_hashes_of_zero() {
        assert_eq!(hash(&0.0f32), hash(&-0.0f32));
    }
    ...
    impl<S: StreamState> Hash<S> for Compound {
        #[inline]
        fn hash(&self, mut state: S) -> u64 {
            self.input(&mut state);
            state.result()
    ...
    impl<S: StreamState> StreamHash<S> for Compound {
        #[inline]
        fn input(&self, state: &mut S) {
            self.x.input(state);
            self.y.input(state);
