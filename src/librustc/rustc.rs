...
pub fn run_compiler(args: &~[~str], demitter: diagnostic::Emitter) {
    ...
    let odir = getopts::opt_maybe_str(matches, "out-dir").map(|o| Path(o));
    let ofile = getopts::opt_maybe_str(matches, "o").map(|o| Path(o));
    let cfg = build_configuration(sess, binary, &input);
    let pretty = getopts::opt_default(matches, "pretty", "normal").map_ref(
