...
pub fn print_expr(s: @ps, expr: @ast::expr) {
    ...
    match expr.node {
      ...
      ast::expr_call(ast::CallFn(func, ref args, sugar)) => {
        let mut base_args = copy *args;
        let blk = print_call_pre(s, sugar, &mut base_args);
        print_expr(s, func);
        print_call_post(s, sugar, &blk, &mut base_args);
      }
      ast::expr_call(ast::CallMethod(_, func, ident, ref tys, ref args, sugar)) => {
        let mut base_args = copy *args;
        let blk = print_call_pre(s, sugar, &mut base_args);
        print_expr(s, func);
        word(s.s, ".");
      ...
      ast::expr_call(ast::CallUnary(_, op, expr)) => {
      ...
      ast::expr_call(ast::CallAssignOp(_, op, lhs, rhs)) => {
        print_expr(s, lhs);
        space(s.s);
      ...
      ast::expr_call(ast::CallIndex(_, expr, index)) => {
        print_expr(s, expr);
        word(s.s, "[");
        print_expr(s, index);
        word(s.s, "]");
