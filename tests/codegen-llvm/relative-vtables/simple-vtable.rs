...
// CHECK-DAG:   define void @{{.*}}13simple_vtable10invoke_foo{{.*}}(ptr noundef nonnull [[DATA:%.*]], ptr noalias noundef readonly align 4 {{.*}}dereferenceable(20) [[VTABLE:%.*]]) {{.*}}{
// CHECK-NEXT:  start:
// CHECK-NEXT:    [[FUNC:%.*]] = tail call ptr @llvm.load.relative.i32(ptr nonnull [[VTABLE]], i32 12), !invariant.load{{.*}}
// CHECK-NEXT:    tail call void [[FUNC]](ptr noundef nonnull [[DATA]]){{.*}}
// CHECK-NEXT:    ret void
// CHECK-NEXT:  }
...
// CHECK-DAG:   define void @{{.*}}13simple_vtable11invoke_drop{{.*}}(i1 noundef zeroext %b) {{.*}}{
// CHECK:         [[BOX:%.*]] = tail call { ptr, ptr } @{{.*}}13simple_vtable13create_struct{{.*}}(i1 noundef zeroext %b)
